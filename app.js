...
client.on("ready", () => { 
    ...
    console.log(`[${formatted}] ログイン完了: ${client.user.tag}`);
    console.log(`[${formatted}] ターゲットユーザーのステータスを取得します...`)
    ...
    console.log(`[${formatted}] 現在のターゲットユーザーのステータスは ${target_user_status} です`)
    ...
    switch (target_user_status) { 
        ...
        console.log(`[${formatted}] BOTのステータスを切り替えました_OFF`);
        break; 
        ...
        console.log(`[${formatted}] BOTのステータスを切り替えました_IDLE`);
        break; 
        ...
        console.log(`[${formatted}] BOTのステータスを切り替えました_DND`);
        break; 
        ...
        console.log(`[${formatted}] BOTのステータスを切り替えました_ON`);
        break; 
...
client.on('presenceUpdate', (oldPresence, newPresence) => { 
    ...
    if(target_user_id == config.TARGET_USER_ID) {
        switch (newPresence.status) { 
            case 'online': 
            console.log(`[${formatted}] ${oldPresence.user.tag} のステータスがオンラインになりました`); 
            client.user.setPresence({ activities: [{ name: `${config.DISCORD_BOT_STATUS_MESSAGE}`, type: ActivityType.Playing }], status: 'invisible' });
            botMember.setNickname(`${config.TARGET_USER_ONLINE_BOT_NICK}`);
            break; 
            
            case 'idle': 
            console.log(`[${formatted}] ${oldPresence.user.tag} のステータスが退席中になりました`); 
            client.user.setPresence({ activities: [{ name: `${config.TARGET_USER_IDLE_BOT_STATUS_MESSAGE}`, type: ActivityType.Playing }], status: 'idle' });
            botMember.setNickname(`${config.TARGET_USER_IDLE_BOT_NICK}`);
            break; 
            
            case 'dnd': 
            console.log(`[${formatted}] ${oldPresence.user.tag} のステータスが取り込み中になりました`); 
            client.user.setPresence({ activities: [{ name: `${config.TARGET_USER_DND_BOT_STATUS_MESSAGE}`, type: ActivityType.Playing }], status: 'dnd' });
            botMember.setNickname(`${config.TARGET_USER_DND_BOT_NICK}`);
            break; 
            
            case 'offline': 
            console.log(`[${formatted}] ${oldPresence.user.tag} のステータスがオフラインになりました`); 
