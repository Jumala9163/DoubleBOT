...
client.on("ready", () => { 
    ...
    switch (target_user_status) { 
        ...
        if (config.CHANGE_BOT_NICK == true) {
            botMember.setNickname(`${config.TARGET_USER_ONLINE_BOT_NICK}`);
        }
        console.log(`[${getNow()}] BOTのステータスを切り替えました_OFF`);
        if (config.NOTIFY_TAGET_USER_STATUS_CHANGE == true) {
            client.channels.cache.get(config.NOTIFY_TAGET_USER_STATUS_CHANGE_CHANNEL_ID).send(`BOT起動完了しました\n現在のステータス[OFF]`);
        }
        break; 
...
client.on('presenceUpdate', (oldPresence, newPresence) => { 
    ...
    if(target_user_id == config.TARGET_USER_ID) {
        switch (newPresence.status) { 
            ...
            if (config.NOTIFY_TAGET_USER_STATUS_CHANGE == true) {
                client.channels.cache.get(config.NOTIFY_TAGET_USER_STATUS_CHANGE_CHANNEL_ID).send(`${oldPresence.user.tag} のステータスがオンラインになりました`);
            }
            client.user.setPresence({ activities: [{ name: `${config.DISCORD_BOT_STATUS_MESSAGE}`, type: ActivityType.Playing }], status: 'invisible' });
            if (config.CHANGE_BOT_NICK == true) {
                botMember.setNickname(`${config.TARGET_USER_ONLINE_BOT_NICK}`);
            ...
            if (config.NOTIFY_TAGET_USER_STATUS_CHANGE == true) {
                client.channels.cache.get(config.NOTIFY_TAGET_USER_STATUS_CHANGE_CHANNEL_ID).send(`${oldPresence.user.tag} のステータスが退席中になりました`);
            }
            ...
            if (config.NOTIFY_TAGET_USER_STATUS_CHANGE == true) {
                client.channels.cache.get(config.NOTIFY_TAGET_USER_STATUS_CHANGE_CHANNEL_ID).send(`${oldPresence.user.tag} のステータスが取り込み中になりました`);
            }
            ...
            if (config.NOTIFY_TAGET_USER_STATUS_CHANGE == true) {
                client.channels.cache.get(config.NOTIFY_TAGET_USER_STATUS_CHANGE_CHANNEL_ID).send(`${oldPresence.user.tag} のステータスがオフラインになりました`);
            }
